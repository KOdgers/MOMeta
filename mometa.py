...
from mobile_blocks import MOMetaBlock, SEBlock
...
class MoMeta(nn.Module):
    ...
    def __init__(self,
        ...
        # Build stages
        self.stage0 = MOMetaBlock(in_channels=3, out_channels=self.in_planes,
    ...
    def _make_stage(self,
        ...
        for ix, stride in enumerate(strides):
            ...
            # Depthwise conv
            blocks.append(MOMetaBlock(in_channels=self.in_planes,
...
def mometa(num_classes: int = 1000, inference_mode: bool = False,
    ...
    return MoMeta(num_classes=num_classes, inference_mode=inference_mode,
                     **variant_params)
...
def condense_model(model: torch.nn.Module) -> nn.Module:
    model = copy.deepcopy(model)
    for module in model.modules():
...
def final_condense_model(model: torch.nn.Module) -> torch.nn.Module:
    model = copy.deepcopy(model)
    for module in model.modules():
        if hasattr(module, 'condense_final'):
            module.condense_final()
    return model
